...
public class DocxToMysqlDDL2 {
    private static final Logger logger = LoggerFactory.getLogger(DocxToMysqlDDL2.class);

    public static void main(String[] args) {
        String directoryPath = "src/main/resources/1114";
            ...
            for (Map.Entry<String, List<Field>> entry : allTableFields.entrySet()) {
                ...
                for (Field field : fields) {
                    if (field.getKey() != null) {
                        ...
                        row.getCell(2).setCellValue(field.getLabel() != null ? field.getLabel() : ""); // 字段描述
                        row.getCell(3).setCellValue(field.getData_type() != null ? field.getData_type() : ""); // 字段类型
        ...
        List<String> fieldDefinitions = fields.stream()
                ...
                .map(field -> {
                    ...
                    if (field.getDescription() != null) {
                        ...
                        fieldDef.append(" COMMENT '").append(field.getLabel().replace("'", "\\'")).append("'");
                    }
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (Field field : fields) {
                if (field.getKey() != null) {
                    ...
                    row.createCell(1).setCellValue(field.getLabel());
                    row.createCell(2).setCellValue(field.getData_type());
    ...
    private static String getMysqlType(Field field) {
        if (field.getData_type() == null) return "VARCHAR(255)";

        switch (field.getData_type().toUpperCase()) {
            case "STRING":
